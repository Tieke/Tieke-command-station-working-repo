function createNewBotButtonHtml() {
  var html = ""
  + "<div class='button-div'>"
  +   '<button type="submit" id="create-bot-button">Create New Bot</button>'
  + "</div>"
  return html
}
...
function createStockpileHtml(stockpile) {
  var html = "<div class='stockpile-div'>"
      ...
      +     "</li>"
      +   "</ul>"
      + createNewBotButtonHtml()
      + "</div>"
      return html
}

function createBotTableHtml(botArry) {
  var html = "<h2 id='bots-table-label'>All Bots</h2>"
    + "<table id='bots-table'>"
    +   "<thead>"
    +     "<tr>"
    +        "<th></th>"
...
html += "<tbody>"
      ...
      +     "<td>"
      //ad in bot id
      +       '<img id="theImg" class="bot-sprite" src="http://arcengames.com/mediawiki/images/3/3a/BDMastermindBotSprite.gif" />'
      +     "</td>"
      +   "</tr>"
      + "</tbody>"
    };
...
function createLandscapeHtml(cellArray) {
  var counter = 0
  var html = "<h2>Landscape</h2>"
           + "<h4>F = Food Available | M = Minerals Available | E = Energy Expenditure</h4>"
           + "<div id='map'>"
  for (var i = 0; i < 10; i++) {
    //row div open
    html += "<div class='row' id='" + (i + 1) + "'>"
    for (var j = 0; j < 10; j++) {
      html += "<div class='cell' id='" + (j + 1) + "'>" //cell div open
      + "<p>F: " + cellArray[counter]["food_count"] + " </p>" //cell information
      + "<p>M: " + cellArray[counter]["mineral_count"] + " </p>"
      + "<p>E: " + cellArray[counter]["energy_expenditure"] + " </p>"
      + "</div>" //cell div close
      counter += 1
    };
...
function createSingleCellHtml(cellObject) {
  var html = "<p>F: "
          + cellObject["food_count"] + " </p>"
          + "<p>M: "
          + cellObject["mineral_count"] + " </p>"
          + "<p>E: "
          + cellObject["energy_expenditure"] + " </p>"
  return html
};
